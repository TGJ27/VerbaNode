...
def test_database_has_numbered_schema_version(tmp_path: Path) -> None:
    ...
    db.initialize()
    assert db.get_setting("schema_version") == "6"
...
def test_v077_hardening_layout_and_ci_are_present() -> None:
    ...
    assert '/static/VerbaNode.png?v=0.9.1' in index_html
    assert 'styles.css?v=0.9.1' in index_html
    assert 'app.js?v=0.9.1' in index_html
    assert 'id="appVersion">v0.9.1<' in index_html
